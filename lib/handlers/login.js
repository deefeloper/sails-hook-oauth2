  ...
  return function (req, res) {
    ...
    function afterClientCheck (err, client, allowedGrantTypes) {
      ...
      if (client.allowedGrantTypes.indexOf(grantType) === -1) {
        return config.response.fail(res, new OAuth2Error('Invalid grant type for client'));
      }
      ...
      if (!username) {
        return config.response.fail(res, new OAuth2Error('Missing username'));
      }

      if (!password) {
        return config.response.fail(res, new OAuth2Error('Missing password'));
      }
    ...
    function afterUserCheck (client, err, user) {
      if (err) {
        return config.response.fail(res, new OAuth2Error('Invalid user credentials'));
      }
